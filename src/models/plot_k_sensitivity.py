...
def export_k_sensitivity_csv(
    ...
    out = empty_schema_df("k_sensitivity")
    write_csv(out, output_csv, schema_name="k_sensitivity")
    write_placeholder_figure(output_fig, "Sensitivity to latent dimension k", "Placeholder figure. Replace with the real sensitivity plot after experiment aggregation is implemented.")
    logger.info("Placeholder CSV and figure written to %s and %s", output_csv, output_fig)


def build_parser() -> argparse.ArgumentParser:
    p = make_parser("Export k-sensitivity CSV and figure. Skeleton only.")
...
def main() -> None:
    ...
    export_k_sensitivity_csv(**vars(args))
